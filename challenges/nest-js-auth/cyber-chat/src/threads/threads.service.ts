import { ForbiddenException, Injectable, NotFoundException } from "@nestjs/common";
import { Thread } from "./entities/thread.entity";
import { MoreThanOrEqual, Repository } from "typeorm";
import { InjectRepository } from "@nestjs/typeorm";
import { UpdateThreadDto } from "./dtos/updateThread.dto";
import { CreateThreadDto } from "./dtos/createThread.dto";
import { PaginationQueryDto } from "../common/dtos/paginationQuery.dto";
import { PaginationMetaResponseDto } from "../common/dtos/paginationMetaResponse.dto";

@Injectable()
export class ThreadsService {
	constructor(
		@InjectRepository(Thread)
		private readonly threads: Repository<Thread>,
	) {}
	async getAllThreads(
		pagination: PaginationQueryDto,
		startDate: Date,
	): Promise<{ data: Thread[]; meta: PaginationMetaResponseDto }> {
		const { page, limit, sort, author } = pagination;

		const [data, total] = await this.threads.findAndCount({
			where: {
				...(author && { author }),
				createdAt: MoreThanOrEqual(startDate),
			},
			order: {
				createdAt: sort === "-createdAt" ? "DESC" : "ASC",
			},
			skip: (page - 1) * limit,
			take: limit,
		});

		return {
			data,
			meta: {
				page,
				limit,
				total,
				totalPages: Math.ceil(total / limit),
			},
		};
		//return this.threads.find();
	}
	async getThreadById(id: string): Promise<Thread> {
		const thread = await this.threads.findOneBy({ id });
		if (!thread) {
			throw new NotFoundException(`Thread with ID "${id}" not found.`);
		}
		return thread;
	}

	async getThreadWithComments(id: string): Promise<Thread> {
		const thread = await this.threads.findOne({
			where: { id },
			relations: {
				comments: true,
			},
		});

		if (!thread) {
			throw new NotFoundException(`Thread with ID '${id}' not found`);
		}

		return thread;
	}

	createThread(threadPayload: CreateThreadDto, author: string): Promise<Thread> {
		const thread = this.threads.create({ ...threadPayload, author });
		return this.threads.save(thread);
	}

	async updateThread(
		id: string,
		threadPayload: UpdateThreadDto,
		author: string,
	): Promise<Thread> {
		const thread = await this.threads.findOneBy({ id });

		if (!thread) {
			throw new NotFoundException(`Thread with ID '${id}' not found`);
		}

		if (thread.author !== author) {
			throw new ForbiddenException("You can only edit your own threads");
		}

		Object.assign(thread, threadPayload);
		return this.threads.save(thread);
	}

	async deleteThreadById(id: string, author: string): Promise<boolean> {
		const thread = await this.threads.findOneBy({ id });

		if (!thread) {
			throw new NotFoundException(`Thread with ID '${id}' not found`);
		}

		if (thread.author !== author) {
			throw new ForbiddenException("You can only delete your own threads");
		}

		const result = await this.threads.delete(id);
		return (result.affected ?? 0) > 0;
	}
}
